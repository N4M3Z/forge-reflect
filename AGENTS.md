...
dependency on `forge-core` (`../../Core`). Works in Claude Code (native hooks),
OpenCode (TypeScript plugin adapter), and standalone CLI.
...
|-----------|-----------------------|------------------------------------------------------|
| `surface` | `src/bin/surface.rs`  | SessionStart digest — resurfaces ideas, tabs, backlog items |
...
.opencode/
└── plugins/
    └── forge-reflect.ts      # OpenCode plugin adapter (bridges Rust binaries to events)
```
...
- Build multi-line output with `String::new()` + `writeln!()` (via `use std::fmt::Write`).
- Prefer `writeln!` over `push_str(&format!(...))` to avoid extra allocation.
...
- 53 unit tests across 5 test locations. No integration tests directory.
...
`Config::load()` and `bin/_build.sh` check env vars in order:
`FORGE_MODULE_ROOT` (tool-agnostic) -> `CLAUDE_PLUGIN_ROOT` (Claude Code) -> self-discovery.
Hook scripts fall back to deriving the root from their own filesystem location.

### OpenCode Plugin Adapter

`.opencode/plugins/forge-reflect.ts` bridges the Rust binaries into opencode's
event-based plugin system. It maps:
- `session.created` -> `surface` binary (TUI toast)
- `experimental.session.compacting` -> `reflect` in PreCompact mode (`output.context`)
- `session.idle` -> `insight` + `reflect` in Stop mode (warning toast, non-blocking)

The adapter sets `FORGE_MODULE_ROOT` when invoking binaries and auto-builds on first use.
