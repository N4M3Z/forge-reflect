...
#[test]
fn tabs_rotation_offset() {
    let content = "- [A](https://a)\n- [B](https://b)\n- [C](https://c)\n";
    let r0 = parse_captured_tabs(content, 1, 0).unwrap();
    let r1 = parse_captured_tabs(content, 1, 1).unwrap();
    assert!(r0.contains('A'));
    assert!(r1.contains('B'));
}

#[test]
fn tabs_empty_returns_none() {
